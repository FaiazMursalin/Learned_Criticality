...
def main(ox, oy,filename, rng=None):
    ...
    sample_x, sample_y = prm_planning(sx, sy, gx, gy, ox, oy, robot_size, rng=rng)

    # save samples coordinates
    with open(filename, 'w') as file:
        # Iterate over coordinates and write to the file
        for i, (x, y) in enumerate(zip(sample_x, sample_y)):
            # Check if it's the last coordinate pair
            if i == len(sample_x) - 1:
                file.write(f'{x},{y}')
            else:
                file.write(f'{x},{y}\n')
...
if __name__ == '__main__':
    ...
    for image_path in image_paths:
        ttv = image_path.split(os.path.sep)[1]
        ...
        for y in range(height):
            for x in range(width):
                if im[y, x] == 0:  # Assuming 1 represents free space
                    x_coords.append(x)
                    y_coords.append(y)

        main(x_coords, y_coords,f"Samples_dataset/{ttv}/{filename}_samples.txt")
